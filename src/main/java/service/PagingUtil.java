...
public class PagingUtil {
	...
	public static String pagingText(int totalRecordCount,int pageSize,int blockPage,int nowPage,String page){
		...
		// 페이지를 뿌려주는 로직
		// 블락 페이지 수만큼 혹은 마지막 페이지가 될 때까지 페이지를 표시
		while(blockCount <= blockPage && intTemp <= totalPage){ // 페이지 오버를 체크
				// 현재 페이지를 의미함
			if(intTemp == nowPage){  
	...
	public static String pagingBootStrapStyle(int totalRecordCount,int pageSize,int blockPage,int nowPage,String page){		
		...
		// 페이지를 뿌려주는 로직
		// 블락 페이지 수만큼 혹은 마지막 페이지가될 때까지 페이지를 표시한다1.
	...
	// 페이징시 필요 속성(키)를 저장하는 맵 설정용 메소드
	// Map map:필요한 키와 값을 담을 빈 맵
	// T dao :  DAO계열을 받는다
	// 제너릭 메소드로 구현 : DAOService 를 상속받은 모든 DAO계열을 인자로 받기위해서 - 중복 코딩 방지
	public static void setMapForPaging(Map map, KosmoDao dao,HttpServletRequest request) {
		// 페이징을 위한 로직 시작
		// 전체 레코드수
